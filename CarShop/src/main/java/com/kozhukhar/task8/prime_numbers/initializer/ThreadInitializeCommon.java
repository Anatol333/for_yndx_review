...
public class ThreadInitializeCommon implements ThreadInitialize {
    ...
    public void initializeThread(
        ...
        for (int i = 0; i < countOfThread - 1; ++i) {
            findPrimeNum[i] = new FindPrimeNum(primeNumbers, i + from, frequency, countOfThread);
            threads[i] = new Thread(findPrimeNum[i]);
            temporaryFrom += frequency;
        }
        findPrimeNum[countOfThread - 1] = new FindPrimeNum(primeNumbers, temporaryFrom, temporaryFrom + lastFrequency);
